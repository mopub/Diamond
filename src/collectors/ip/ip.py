...
class IPCollector(diamond.collector.Collector):
    ...
    def get_default_config(self):
        ...
        config.update({
            'path':             'ip',
            'allowed_names':    'InReceives, InHdrErrors, InAddrErrors, ForwDatagrams, InUnknownProtos, '
            + 'InDiscards, OutRequests, OutDiscards, OutNoRoutes, ReasmTimeout, ReasmReqds, ReasmOKs, '
    ...
    def collect(self):
        ...
        for filepath in self.PROC:
            ...
            if header == '' or data == '':
                self.log.error('%s has no lines with Ip', filepath)
                continue
